...
const GroomingBookingForm: React.FC<GroomingBookingFormProps> = ({
      ...
      const bookingResults = await Promise.all(
        bookings.map(async (booking) => {
          ...
          if (booking.pet.pet_type === 'cat') {
            basePrice = pricing.grooming.cat.standard;
          } else {
            basePrice = pricing.grooming.dog[booking.pet.service_variant as DogGroomingVariant]?.[booking.pet.size as PetSize] || 0;
          }
  ...
  const getServiceVariantOptions = (petType: PetType) => {
    if (petType === 'cat') {
      return [
        { value: 'standard', label: 'Standard Grooming' }
      ];
    }
    return [
              ...
              <div className="space-y-4 md:col-span-2">
                ...
                <div className="space-y-2">
                  ...
                  <div className="block w-full px-3 py-2 border border-gray-200 bg-gray-50 rounded-md sm:text-sm">
                    {getSizeDisplay(pet.size, pet.pet_type)}
                  </div>
